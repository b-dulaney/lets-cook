...
function RecipeDetailContent() {
  ...
  const fetchRecipeDetails = async () => {
    ...
    try {
      ...
      if (cookTimeParam) requestBody.cookTime = cookTimeParam;
      if (difficultyParam) requestBody.difficulty = difficultyParam;
      if (servingsParam) requestBody.servings = parseInt(servingsParam);

      const res = await fetch("/api/recipes/details", {
        method: "POST",
      ...
      if (res.ok) {
        ...
        if (data.recipeId && mealPlanId && dayIndexParam !== null) {
          try {
            await fetch(`/api/meal-plans/${mealPlanId}/recipes`, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
                dayIndex: parseInt(dayIndexParam),
                recipeId: data.recipeId,
              }),
            });
          } catch (linkErr) {
            console.error("Error linking recipe to meal plan:", linkErr);
            // Non-fatal error - recipe still displayed
          }
        }
      } else {
        const data = await res.json();
  ...
  const createShoppingList = async () => {
    ...
    try {
      const res = await fetch("/api/shopping-lists", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ generate: true, recipeId }),
      });
    ...
    return (
      <>
        ...
        <FullPageLoader
          ...
          submessage={`Getting the full recipe for ${recipeName}`}
        />
      </>
    );
  }
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="mt-2 text-gray-600">
          We couldn&apos;t load the recipe details. Please try again.
        </p>
        <button
          onClick={() => router.back()}
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-lg border border-gray-200 p-5 lg:sticky lg:top-4">
            ...
            {/* Create Shopping List button - only show when recipe is saved */}
            {recipeId && (
              <button
                onClick={createShoppingList}
                disabled={creatingList}
                className="mt-6 w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-gray-100 text-gray-700 rounded-lg font-medium hover:bg-gray-200 transition-colors disabled:opacity-50 cursor-pointer"
              >
                {creatingList ? (
                  <>
